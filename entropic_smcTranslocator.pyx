...
cdef class smcTranslocatorDirectional(object):
    ...
    def __init__(self, emissionProb, deathProb, stallProbLeft, stallProbRight, pauseProb, stallFalloffProb,  
                 numSmc, onesided, paired=0, switch=0, 
    ...
    def set_slidepause(self, slidepauseForward, slidepauseBackward):
        self.slidepauseForward=slidepauseForward
        self.slidepauseBackward=slidepauseBackward
    ...
    cdef step(self):
        ...
        for i in range(self.M):            
                ...
                if self.occupied[cur1-1] == 0:
                    ...
                    while pushing_allowed and self.occupied[x]: # while we need to check obstacle lefs
                        if x-1<0: # if lef at x can't move, can't push
                            ...
                            if len(y)>0: 
                                if self.stalled1[y[0]] and (not (self.stallLeft[x]>0.0)) and (not self.belt_attached[y[0]]): 
                                ...
                                if self.stalled2[y[0]] and (not (self.stallRight[x]>0.0)) and (not (self.stallLeft[x]>0.0)) and not self.belt_attached[y[0]]: 
                                    x=x-1
                                else:
                                    pushing_allowed=0
                                    break
            ...
            elif self.slide and (not self.belt_attached[i]) and (self.inactive_side[i]==1):
